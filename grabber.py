...
class TelegramGrabber:
    ...
    async def download_media(self, url: str, progress_callback=None) -> dict:
        ...
        try:
            # Таймаут на всю операцию получения сообщения
            async with asyncio.timeout(30):
                ...
                if isinstance(raw_chat_id, str) and raw_chat_id.isdigit():
                    # Сначала пробуем самый вероятный вариант -100...
                    id_variants = [f"-100{raw_chat_id}", f"-{raw_chat_id}"]
                    
                    for attempt_id in id_variants:
                        try:
                            chat_id = int(attempt_id)
                            # get_messages с коротким таймаутом
                            message = await asyncio.wait_for(self.client.get_messages(chat_id, message_id), timeout=5)
                            if message and not message.empty:
                                break
                        except (asyncio.TimeoutError, Exception):
                            continue
                    
                    # Если всё еще нет сообщения, пробуем диалоги, но с лимитом и таймаутом
                    if not message:
                        try:
                            # Ограничиваем время поиска в диалогах
                ...
                if not message or message.empty:
                    result["error"] = "❌ Сообщение не найдено.\nУбедитесь, что ссылка верная и аккаунт подписан на канал."
                    return result

                # Проверяем, часть ли это медиагруппы
                if message.media_group_id:
                    messages = await asyncio.wait_for(self.client.get_media_group(chat_id, message_id), timeout=10)
                else:
                    messages = [message]
